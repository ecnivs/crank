...
class TestGemini:
    ...
    def test_get_audio_success(self, mock_gemini_client, temp_dir):
        ...
        def generate_content_side_effect(model, contents, config=None):
            if config and hasattr(config, 'response_modalities') and "AUDIO" in str(config.response_modalities):
        ...
        mock_gemini_client.models.generate_content.side_effect = generate_content_side_effect
    ...
    @patch("response.gemini.time.sleep")
    def test_get_response_fallback_model(self, mock_sleep, mock_gemini_client, temp_dir):
    ...
    @patch("response.gemini.time.sleep")
    def test_get_response_quota_exceeded(self, mock_sleep, mock_gemini_client, temp_dir):
