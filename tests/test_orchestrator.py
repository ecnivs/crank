...
def mock_orchestrator_dependencies(temp_dir, preset_handler, mock_gemini_client):
    ...
    with patch("orchestrator.Scraper") as mock_scraper_class, \
...
class TestOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_process_without_uploader(self, preset_handler, mock_orchestrator_dependencies):
    ...
    @pytest.mark.asyncio
    async def test_process_quota_exceeded(self, orchestrator, mock_orchestrator_dependencies):
        ...
        with patch("orchestrator.Prompt") as mock_prompt_class:
            ...
            deps["gemini"].get_response.side_effect = QuotaExceededError("Quota exceeded")
    ...
    @pytest.mark.asyncio
    async def test_process_missing_transcript(self, orchestrator, mock_orchestrator_dependencies):
    ...
    @pytest.mark.asyncio
    async def test_upload_resumable_error(self, orchestrator, mock_orchestrator_dependencies):
